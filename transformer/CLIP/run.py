...
import yaml
import argparse
from contrastive_loss import contrastive_clip_loss_function
...
def run(args):
    ...
    for  epoch in range(5):
        ...
        for idx,batch in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            loss = contrastive_clip_loss_function(image_projections, text_projections, temperature_value=0.7)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
